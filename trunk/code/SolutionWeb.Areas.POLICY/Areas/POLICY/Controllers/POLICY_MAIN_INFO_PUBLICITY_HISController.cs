...
{
    public class POLICY_MAIN_INFO_PUBLICITY_HISController : BaseController
    {
        // GET: POLICY/POLICY_MAIN_INFO_PUBLICITY_HIS
        #region Identity
        private IBaseService bs = null;
        private IUserLogin ul = null;

        public POLICY_MAIN_INFO_PUBLICITY_HISController(IBaseService baseService, IUserLogin userLogin)
        {
            this.bs = baseService;
            this.ul = userLogin;
        }
        ...
        public ActionResult Index(string id)
        {
            ...
            string ControllerUrl = "/api/POLICY/POLICY_MAIN_INFO_PUBLICITY_HIS/";
            var viewModel = new
            {
                Permission = new//权限
                {
                    a_list = this.ul.HasPermission("POLICY", "POLICY_MAIN_INFO_PUBLICITY_HIS", "List", Common.HttpMethod.Post),
                    //a_selectdel = this.ul.HasPermission("POLICY", "POLICY_MAIN_INFO_PUBLICITY_HIS", "List", Common.HttpMethod.Post),
                },
                resx = new
                {
    ...
    public class POLICY_MAIN_INFO_PUBLICITY_HISApiController : BaseApiController
    {
        #region Ioc
        private IBaseService bs = null;
        public POLICY_MAIN_INFO_PUBLICITY_HISApiController(IBaseService baseService)
        {
            this.bs = baseService;
        }
        #endregion
        ...
        public ViewModel List(VIEW_POLICY_STATUS_CHANGE data)
        {
            ...
            IQueryable<POLICY_STATUS_CHANGE> statusEntites =
                bs.Entities<POLICY_STATUS_CHANGE>()
                    .Where(
                        o =>
                            o.DEPT_CODE.Equals(oc.CurrentUser.DEPT_CODE) && o.IS_HANDLE == "1" &&
                            o.STATUS_CODE == Constant.POLICY_STATUS.事项公示);
        ...
        #region 公示操作，暂停、终止、继续
        [HttpPost]
        public AjaxMsgModel PolicityStatusChange(POLICY_MAIN_INFO model)
        {
            return Model_POLICY_MAIN_INFO.Create.SavePolicityStatus(model);
        }
        #endregion
    }
}