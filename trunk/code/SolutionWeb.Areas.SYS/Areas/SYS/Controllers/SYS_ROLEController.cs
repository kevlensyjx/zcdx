...
namespace SolutionWeb.Areas.SYS.Controllers
{
    public class SYS_ROLEController : BaseController
    {
        ...
        public SYS_ROLEController(IBaseService baseService)
        {
            this.bs = baseService;
        }
        #endregion
        ...
        public ActionResult Index()
        {
            ...
            var viewModel = new
            {
                Permission = new//权限
                {
                    a_list = Model_SYS_MENU.Create.HasPermission("SYS", "SYS_ROLE", "List", HttpMethod.Post),
                    a_add = Model_SYS_MENU.Create.HasPermission("SYS", "SYS_ROLE", "Add", HttpMethod.Post),
                    a_edit = Model_SYS_MENU.Create.HasPermission("SYS", "SYS_ROLE", "Edit", HttpMethod.Get),
                    a_del = Model_SYS_MENU.Create.HasPermission("SYS", "SYS_ROLE", "Del", HttpMethod.Get),
                },
                resx = new
                {
        ...
        public ActionResult Save()
        {
            try
            {
                ...
                {
                    //最后一个role_id
                    var lastCode = bs.Entities<SYS_ROLE>().OrderByDescending(m => m.ROLE_ID).Select(m => new { m.ROLE_ID }).FirstOrDefault();
                    int lastCodeId = 1;
                    if (lastCode != null)
                    {
                ...
                if (menu != "")
                {
                    ...
                    //    //}
                    //listChildMenuId = bs.Entities<SYS_MENU>().Where(m => listTwoMenuId.Contains(m.PARENT_ID)).Select(m => m.MENU_ID).ToList();
                ...
                if (upid == "")
                {
                    return PackagingAjaxmsg(Model_SYS_ROLE.Create.Add(u));
                }
                else
                {
                    u.ROLE_ID = upid;
                    return PackagingAjaxmsg(Model_SYS_ROLE.Create.Edit(u));
                }
    ...
    public class SYS_ROLEApiController : BaseApiController
    {
        #region Ioc
        private IBaseService bs = null;
        public SYS_ROLEApiController(IBaseService baseService)
        {
            this.bs = baseService;
        }
        #endregion
        ...
        public List<EasyUITreeNode> GetMyMenuListTree()
        {
            ...
            //获得组织
            List<SYS_MENU> listMenu = bs.Entities<SYS_MENU>().Where(m => listMuenId.Contains(m.MENU_ID) && m.MENU_LEVEL <= 3).OrderBy(m => m.MENU_ID).OrderBy(m => m.MENU_ORDER).ToList();
            return Model_SYS_MENU.Create.ConvertTreeNodes(listMenu, "0");
        }
        #endregion
        ...
        public ViewModel List(VIEW_SYS_ROLE data)
        {
            ...
            IQueryable<SYS_ROLE> SYS_ROLEEntity = bs.Entities<SYS_ROLE>();
        ...
        public AjaxMsgModel Edit(VIEW_SYS_ROLE data)
        {
            ...
            //{
                ...
                VIEW_SYS_ROLE role = bs.Entities<SYS_ROLE>().Where(u => u.ROLE_ID == data.ROLE_ID).OrderBy(u => u.ROLE_ID)
                        .Select(u =>
                            new VIEW_SYS_ROLE
                            {
                                ROLE_ID = u.ROLE_ID,
                ...
                if (role != null)
                {
                    List<string> listMenuId = bs.Entities<SYS_ROLE_MENU_MAP>().Where(u => u.ROLE_ID == data.ROLE_ID && u.MENU_ID.Length < 7).Select(u => u.MENU_ID).ToList();
                    role.MenuId = listMenuId;
                    List<string> listMenuOptId = bs.Entities<SYS_ROLE_MENUOPT_MAP>().Where(u => u.ROLE_ID == data.ROLE_ID && u.MENUOPT_ID.EndsWith("004")).Select(u => u.MENUOPT_ID.Substring(0, 6)).ToList();
                ...
                if (role != null)
                {
                    List<string> listMenuId = bs.Entities<SYS_ROLE_MENU_MAP>().Where(u => u.ROLE_ID == data.ROLE_ID).Select(u => u.MENU_ID).ToList();
                    role.MenuId = listMenuId;
                    List<string> listMenuOptId = bs.Entities<SYS_ROLE_MENUOPT_MAP>().Where(u => u.ROLE_ID == data.ROLE_ID).Select(u => u.MENUOPT_ID).ToList();
                    role.MenuOptId = listMenuOptId;
                    amm.Statu = AjaxStatu.ok;
                    amm.Data = role;
                    return amm;
        ...
        {
            return Model_SYS_ROLE.Create.Del(data.ROLE_ID);
        }
        #endregion
