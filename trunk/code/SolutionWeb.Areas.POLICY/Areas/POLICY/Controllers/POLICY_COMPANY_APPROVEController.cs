...
{
    public class POLICY_COMPANY_APPROVEController : BaseController
    {
        #region Identity
        private IBaseService bs = null;
        private IUserLogin ul = null;

        public POLICY_COMPANY_APPROVEController(IBaseService baseService, IUserLogin userLogin)
        {
            this.bs = baseService;
            this.ul = userLogin;
        }
        #endregion
        // GET: POLICY/POLICY_COMPANY_APPROVE
        public ActionResult Index()
        {
            string ControllerUrl = "/api/POLICY/POLICY_COMPANY_APPROVE/";
            var viewModel = new
            {
                Permission = new//权限
                {
                    a_list = this.ul.HasPermission("POLICY", "POLICY_COMPANY_APPROVE", "List", Common.HttpMethod.Post),
                },
                resx = new
                {
    ...
    public class POLICY_COMPANY_APPROVEApiController : BaseApiController
    {
        #region Ioc
        private IBaseService bs = null;
        public POLICY_COMPANY_APPROVEApiController(IBaseService baseService)
        {
            this.bs = baseService;
        }
        #endregion
        ...
        public AjaxMsgModel GetCorpInfo(VIEW_CORPORATION_BASE_INFO data)
        {
            ...
            try
            {
                ...
                if(info != null)
                {
                    ...
                    if (zzjg != null) u.zzjg = ConfigurationManager.AppSettings["COMPANY_PATH"] + zzjg.FILE_PATH;
                    if (yyzz != null) u.yyzz = ConfigurationManager.AppSettings["COMPANY_PATH"] + yyzz.FILE_PATH;
