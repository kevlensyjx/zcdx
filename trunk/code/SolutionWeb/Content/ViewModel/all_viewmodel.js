...
var viewModel = function (data) {
            ...
            onAfterEdit: function (rowIndex, rowData, changes) {
                if (JSON.stringify(changes) != "{}") {
                    ...
                    function (xhr, textStatus, err) {
                        $('#' + data.urls.dataGgridName)[data.urls.dataGgridType]('loaded');
                        $.alertMsg('修改失败，原因如下: ' + xhr.responseJSON.ExceptionMessage, '操作提示', null);
                    });
                }
            }
        });
    ...
    self.isLogin = function (funcSuc) {
        ...
        function (xhr, textStatus, err) {
            $.alertMsg('提交失败，原因如下: ' + xhr.responseJSON.ExceptionMessage, '操作提示', null);
        });
    }
    //加载权限
                ...
                onSubmit: function () {
                    if ($(this).form('validate')) {
                        if (typeof loadExt == "function") {//存在且是function
                            ...
                            if (self.saveFlag) {//阻断提交,自控提交
                                self.saveCallback();
                            }
                            self.saveFlag = true;//还原
                        } else {
                            self.saveCallback();
                        }
                        return false;
            ...
            if (row) {
                $.messager.confirm('请确认', '您确定要删除选择的行吗！', function (b) {
                    if (b) {
                        ...
                        function (xhr, textStatus, err) {
                            $.alertMsg('删除失败，原因如下: ' + xhr.responseJSON.ExceptionMessage, '操作提示', null);
                        });
                    }
                });
                ...
                if (PKID != "") {
                    $.messager.confirm('请确认', '您确定要删除选中的行吗？', function (b) {
                        if (b) {
                            ...
                            function (xhr, textStatus, err) {
                                $.alertMsg('删除失败，原因如下: ' + xhr.responseJSON.ExceptionMessag, '操作提示', null);
                            });
                        }
                    });
            ...
            self.isLogin(function () {
                ...
                $.messager.confirm('请确认', '您确定要导出数据吗？', function (b) {
                    if (b) {
                        ...
                        function (xhr, textStatus, err) {
                            $.alertMsg('导出失败，原因如下: ' + xhr.responseJSON.ExceptionMessage, '操作提示', null);
                        });
                    }
                });
            });
    ...
    self.selectExcelHandle = function () {
           if ($('#excelBut').linkbutton('options').disabled == false) {//重复提交
...
function stationMap(CRITERION_STATION, CUSTOM_STATION) {
    ...
    $.messager.confirm('请确认', '您确定要与站名 ' + CRITERION_STATION + ' 匹配吗？', function (b) {
        if (b) {
            ...
            function (xhr, textStatus, err) {
                $.alertMsg('匹配失败，原因如下: ' + xhr.responseJSON.ExceptionMessag, '操作提示', null);
            });
        }
    });
}
